	...
	for n in n_list:
		for m in m_list:
			for stop in stop_pct_list:
				...
				#存储数据
				rtn.loc[str(para) + ',' + rule_type + ',' + is_blow_up, '全部回测收益'] = df.iloc[-1]['equity_curve']
	...
	for n in n_list:
		for m in m_list:
			for stop in stop_pct_list:
				...
				#存储数据
				rtn.loc[str(para) + ',' + rule_type + ',' + is_blow_up, '70回测收益'] = df.iloc[-1]['equity_curve']
	...
	for n in n_list:
		for m in m_list:
			for stop in stop_pct_list:
				...
				#存储数据
				rtn.loc[str(para) + ',' + rule_type + ',' + is_blow_up, '30验证收益'] = df.iloc[-1]['equity_curve']
